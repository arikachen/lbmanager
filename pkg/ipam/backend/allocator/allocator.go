...
func (a *IPAllocator) Get(id string, requestedIP net.IP, nogw bool) (*types.IPConfig, []*types.Route, error) {
	a.store.Lock()
...
func (a *IPAllocator) getSearchRange() (net.IP, net.IP) {
	...
	if err != nil && (!(os.IsNotExist(err) || err == store.ErrKeyNotFound)) {
...
func (a *IPAllocator) GetExist(id string) (net.IP, error) {
	a.store.Lock()
	defer a.store.Unlock()

	return a.store.Exist(id)

}

func (a *IPAllocator) Recover() (map[string]string, error) {
	a.store.Lock()
	defer a.store.Unlock()

	return a.store.Recover()
